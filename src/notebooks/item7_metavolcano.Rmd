...
outdir = file.path(basedir,"intermediate/item5_metavolcano")
...
Input
```{r}
fnames <- list.files('intermediate/item6_DEtables/', full.names = T)
...
for(comp in comparisons) {# comp <- "NEG_M"
  ...
  temp <- de.list[grep(comp, de.list$group_name),] %>% select(gene_id,hgnc_symbol, logFC, PValue, dataset) 
