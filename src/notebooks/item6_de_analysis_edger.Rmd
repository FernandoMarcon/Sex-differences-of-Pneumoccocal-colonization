...
de.tables$symbol[which(is.na(de.tables$symbol))] = de.tables$gene_id[which(is.na(de.tables$symbol))]
de.tables$symbol[which(de.tables$symbol == '')] = de.tables$gene_id[which(de.tables$symbol == '')]
```
Number of repeated gene ids by group
```{r}
# de.tables %>% group_by(group_name) %>% summarise(sum(duplicated(gene_id))) %>% View
# de.tables %>% group_by(group_name) %>% summarise(sum(duplicated(symbol))) %>% View
table(is.na(de.tables$symbol))
table(de.tables$symbol == '')
head(de.tables)
```
